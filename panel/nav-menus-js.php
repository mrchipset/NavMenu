<script src="<?php $options->PluginUrl($pluginName);
echo '/panel/js/jquery-ui-1.10.0.custom.min.js'; ?>"></script>
<script src="<?php $options->PluginUrl($pluginName);
echo '/panel/js/jquery.mjs.nestedSortable.js'; ?>"></script>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(document).on('click', '.del_menu', function () {
            let menu = $(this).parent().data('menu');
            if (menu == 'default') {
                alert("<?php _e("不能删除默认菜单"); ?>")
            } else {
                if (confirm("<?php _e("是否删除菜单[%s]"); ?>".replace('%s', menu))) {
                    window.location.href = $('#menu-form').attr('action') + '&do=del-menu&nav_menu=' + menu;
                }
            }
            return false;
        });
        ...
        $(document).on('click', '.save_menu_item', function () {
            ...
            if (($link_name.val() !== undefined && $link_name.val() !== "") || ($link_url.val() !== undefined && $link_url.val() !== "")) {
                ...
                setTimeout(function () {
                    $item.find('.menu-item-handle').removeClass('item_saved');
                }, 1000);
                updateMenuOrder();
